...
def test_get_today_returns_zeros_on_empty_db(client: TestClient) -> None:
...
def test_get_today_reflects_completed_commitment(client: TestClient) -> None:
    """After completing a commitment via PATCH, the stats reflect it."""
    # Create + complete
    created = client.post(
        "/commitments", json={"text": "Done task", "due_at": None}
    ).json()
    client.patch(f"/commitments/{created['id']}", json={"status": "done"})
