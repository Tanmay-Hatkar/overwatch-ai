 ...
 * relay it live so ringAlarm.js's shared handler can react immediately (mark
 * the commitment done, cancel a still-pending sibling alarm, etc).
 *
 * Declared with android:exported="false" in the merged manifest; every
 * sender in this module scopes its Intent with setPackage(), so this only
 * ever receives from the consuming app itself.
 */
