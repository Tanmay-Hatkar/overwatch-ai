 ...
 * calls into (declared as a `<receiver>` in the module's AndroidManifest.xml,
 * pointing at res/xml/commitment_widget_info.xml).
 *
 ...
 * on a home screen; [onDisabled] fires once, when the LAST instance is
 * removed. That makes them the right place to start/stop the periodic
 * WorkManager job — no point polling the backend when no widget is on
 * screen to show the result.
 */
class CommitmentWidgetReceiver : GlanceAppWidgetReceiver() {
     ...
     * any shorter interval up, so there's no fresher option here without a
