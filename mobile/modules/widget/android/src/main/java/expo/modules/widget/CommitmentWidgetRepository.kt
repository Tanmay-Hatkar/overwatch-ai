 ...
 * knows how to turn "a stored config" into "a list of open commitments."
 *
 * No JSON/HTTP library dependency is added for this — org.json and
 * HttpURLConnection are both part of the Android SDK already.
 */
class CommitmentWidgetRepository(private val context: Context) {
     ...
     *
     * Never throws — every external-call failure path (missing config,
    ...
    fun fetchOpenCommitments(): WidgetFetchResult {
        val config = WidgetConfigStore.read(context) ?: return WidgetFetchResult.AuthError

        var connection: HttpURLConnection? = null
        return try {
            val url = URL(config.baseUrl + ENDPOINT_PATH)
     ...
     * naive (offset-less) due_at, so this falls back to treating it as UTC
     * rather than dropping the row's time entirely.
     */
