 ...
 * see CommitmentWidgetReceiver.enqueuePeriodicRefresh), plus an immediate
 * one-off run right after sign-in (see WidgetModule.configure). Each run:
 * fetch via [CommitmentWidgetRepository], keep the 3 nearest-due items, and
 * persist them into every placed widget instance's Glance state so the
 * widget always has last-known-good data to render even when a given run
 * fails (offline, backend down, expired token).
 */
            ...
            updateAppWidgetState(applicationContext, PreferencesGlanceStateDefinition, glanceId) { prefs ->
                ...
                when (fetchResult) {
                    ...
                    WidgetFetchResult.NetworkError -> {
                        ...
                        // last successful fetch — always render last-known-
                        // good data (ADR-0020).
                    }
                }
                mutablePrefs
